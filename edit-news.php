...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update = $conn->prepare("UPDATE news SET title=?, content=?, category=?, image_path=? WHERE id=?");
    $update->bind_param("ssssi", $title, $content, $category, $image_path, $id);
    $update->execute();
    ...
    <div class="from-container">
        <form action="update-news.php" method="POST" enctype="multipart/form-data">
            ...
            <select name="category" required>
                ...
                <option value="destinacionet" <?= $news['category'] == 'destinacionet' ? 'selected' : '' ?>>Destinacionet</option>
            </select><br><br>
